...
def findArea(image: np.ndarray):
    ...
    # the largest contour is our area
    largestContours = sorted(contours, key=cv2.contourArea, reverse=True)[:1]

    # get approximate square of area
    # I need only the Sudoku area (square with four corner)
...
def extract_digit(image, kernal_size=(5, 5)):
    original = image.copy()
    image = image.copy()
...
def find_sudoku_solution(sudoku):
    solved = sudoku.copy()
